...
public final class MailSettingsLoader {
    ...
    public static MailSettings load() {
        ...
        if (!loaded) {
            // Fallback pratique quand les resources ne sont pas sur le classpath (run IDE / JavaFX).
            ...
            for (Path candidate : candidates) {
                ...
                try (InputStream in = Files.newInputStream(candidate)) {
                    properties.load(new java.io.InputStreamReader(in, StandardCharsets.UTF_8));
                    loaded = true;
                    System.out.println("[MAIL] mail.properties chargé depuis: " + candidate.toAbsolutePath());
                    break;
                } catch (IOException e) {
        ...
        // Fallback env vars (pratique pour CI / machines)
        enabled = enabled || getEnvBoolean("PI_MAIL_ENABLED", false);
        host = fallback(host, System.getenv("PI_MAIL_SMTP_HOST"));
        port = fallback(port, System.getenv("PI_MAIL_SMTP_PORT"));
        startTls = startTls || getEnvBoolean("PI_MAIL_SMTP_STARTTLS", false);
        ssl = ssl || getEnvBoolean("PI_MAIL_SMTP_SSL", false);
        debug = debug || getEnvBoolean("PI_MAIL_DEBUG", false);
        debugPopup = debugPopup || getEnvBoolean("PI_MAIL_DEBUG_POPUP", false);
    ...
    private static String fallback(String currentValue, String envValue) {
    ...
    private static int fallback(int currentValue, String envValue) {
    ...
    private static String normalizeSecret(String value) {
        ...
        // Google "App Password" est souvent copié avec des espaces (xxxx xxxx xxxx xxxx).
